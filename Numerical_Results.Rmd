...
Explain why we reject the null hypothesis if the test statistic observed is larger than the critical value (in term of maths)
...
```{r tool1, eval=TRUE, include=FALSE}
...
```{r tool2, eval=TRUE, include=FALSE}
...
boswell_survival_func <- function(w, n) {
  total_sum <- 0
  for (k in 1:n) {
    total_sum <- total_sum + ((1 - pchisq(q=w, df=k + 1)) * (stirling_number(n, k) / factorial(n)))
  }
  return(total_sum)
}
```
...
```{r empirical_alpha, eval=TRUE, include=FALSE}
...
```{r Danish_fit, eval=TRUE, echo=FALSE}
...
```{r Danish_autocorr, eval=TRUE, echo=FALSE}
...
We clean the data by removing duplicate time values and computing the new time intervals.

```{r Danish_clean, eval=TRUE}
# Cleaning the data
...
T_max <- max(times)
n <- length(new_times_intervals)
```

```{r tool3, eval=TRUE, include=FALSE}
# Function to generate a matrix of log Stirling numbers of the first kind
generate_log_stirling_matrix <- function(n_max) {
...
```{r tool4, eval=TRUE, include=FALSE}
boswell_log_survival_func <- function(w, n) {
  # Ensure n does not exceed the precomputed matrix size
  log_stirling_matrix <- generate_log_stirling_matrix(n)

  # Compute the log survival function
  log_total_sum <- -Inf  # Initialize to log(0)

  for (k in 1:n) {
    log_term <- log(1 - pchisq(w, df = k + 1)) + log_stirling_matrix[n, k] - log_factorial(n)
    log_total_sum <- log(exp(log_total_sum) + exp(log_term))
  }

  return(exp(log_total_sum))  # Convert back from log scale
}
```
